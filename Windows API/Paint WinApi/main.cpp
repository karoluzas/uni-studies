...
typedef int (WINAPI *AnotherWindowFunction)(HINSTANCE, HINSTANCE, LPSTR, int, HWND);
LRESULT CALLBACK WindowProcedure (HWND, UINT, WPARAM, LPARAM);
void SaveImage(HWND hwnd);
void LoadPixels(HWND hwnd);
BOOL LoadColorPickerDLL();

TCHAR szClassName[ ] = _T("paintexe");
DrawingState drawingState;
HINSTANCE thisInstance;
HINSTANCE hColorPickerDLL = NULL;
HWND hMainWindow;
POINT prevPos;
HPEN hOldPen;
HPEN hPen;
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    AnotherWindowFunction colorPickerWindow = NULL;
    switch (message)
    {
        case WM_CREATE:
            hMainWindow = hwnd;
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDM_TOOL_BRUSH_CUSTOMIZATION:
                    ...
                    hColorPickerDLL = LoadLibrary(_T("ColorPickerDLL.dll"));

                    if (hColorPickerDLL == NULL) {
                        FreeLibrary(hColorPickerDLL);
                        break;
                    }

                    // Used function is 'AnotherWindow@20' because using dumpbin that what it says the dll exports it as
                    colorPickerWindow = (AnotherWindowFunction) GetProcAddress(hColorPickerDLL, "AnotherWindow@20");
                    if (colorPickerWindow == NULL) {
                        FreeLibrary(hColorPickerDLL);
                        break;
                    }

                    colorPickerWindow(thisInstance, NULL, NULL, TRUE, hwnd);
                    FreeLibrary(hColorPickerDLL);
                    break;
                case IDM_TOOL_LINE:
